...
public class CefApp extends CefAppHandlerAdapter {
    ...
    public static void setWindowingToolkit(WindowingToolkit w) {
        wt = w;
    }
    ...
    public Set<CefClient> getAllClients() {
        return new HashSet<CefClient>(clients_);
    }
    ...
    public synchronized<T extends CefClient> T createClient() {
        switch (getState()) {
            ...
            case INITIALIZING:
            case INITIALIZED:
                CefClient client = wt.createClient();
                clients_.add(client);
                if (!settings_.multi_threaded_message_loop && !settings_.external_message_pump && clients_.size() == 1) {
                    wt.startMessageLoopTimer(wt.getLoopTime(), () -> N_DoMessageLoopWork());
                }
                return (T) client;
    ...
    protected final synchronized void clientWasDisposed(CefClient client) {
        ...
        if (!settings_.external_message_pump && clients_.isEmpty() && !OS.isMacintosh()) {
                    ...
                    if (OS.isMacintosh()) {
                        if (settings.browser_subprocess_path == null) {
                            Path path = Paths.get(library_path,
                                    "../Frameworks/equochro Helper.app/Contents/MacOS/equochro Helper");
                            settings.browser_subprocess_path =
                                    path.normalize().toAbsolutePath().toString();
                        }
                    } else if (OS.isWindows()) {
                        if (settings.browser_subprocess_path == null) {
                            Path path = Paths.get(library_path, "equochro_helper.exe");
                            settings.browser_subprocess_path =
                                    path.normalize().toAbsolutePath().toString();
                        }
                    } else if (OS.isLinux()) {
                        if (settings.browser_subprocess_path == null) {
                            Path path = Paths.get(library_path, "equochro_helper");
                            settings.browser_subprocess_path =
                                    path.normalize().toAbsolutePath().toString();
                        }
    ...
    public final void doMessageLoopWork(final long delay_ms) {
        ...
        if (settings_.multi_threaded_message_loop)
            return;
        if (!settings_.external_message_pump) {
            wt.startMessageLoopTimer((int) delay_ms, () -> N_DoMessageLoopWork());
            return;
        }
        Runnable run = new Runnable() {
            ...
            public void run() {
                ...
                final long kMaxTimerDelay = wt.getLoopTime(); // 30fps
                ...
                if (delay_ms <= 0) {
                    ...
                    wt.startMessageLoopTimer((int) timer_delay_ms, new Runnable() {
                        @Override
                        public void run() {
                            // Timer has timed out.
                            wt.stopMessageLoopTimer();
    ...
    /**
     * Runs the CEF loop. This method does not return until the browser is closed.
     */
    ...
    public static final boolean startup(String[] args) {
        if (OS.isLinux() || OS.isMacintosh()) {
            SystemBootstrap.loadLibrary("equochro");
            return N_Startup(OS.isMacintosh() ? getCefFrameworkPath(args) : (OS.isLinux() ? getLinuxFlags(args) : null));
        }
        return true;
    }
        ...
        for (String path : paths) {
            ...
            String[] found = dir.list(new FilenameFilter() {
                ...
                public boolean accept(File dir, String name) {
                    return (name.equalsIgnoreCase("libequochro.dylib")
                            || name.equalsIgnoreCase("libequochro.so")
                            || name.equalsIgnoreCase("equochro.dll"));
                }
            });
    ...
    private static String getLinuxFlags(String[] args) {
        String extras = "";
        for (String arg : args) {
            if (arg.contains("XInitThreads")) {
                extras += "XInitThreads ";
            }
            if (arg.contains("GTKInitCheck")) {
                extras += "GTKInitCheck ";
            }
        }
        return extras;
